...
这些信息已经足够把地图从粗标签转成 [[syntheses/BP-地图建模与决策规范|BP 地图建模与决策规范]] 中的 `map_feature` 候选。2026-06-29 的二次 ingest 已进一步把 26 张地图页从 `first_pass_from_fandom_text` 升级为 `bp_map_profile_v2`，重点补充“能力 -> 路线 / 位置 -> 目标收益 -> 失效条件 -> BP 用途”的中间表达。
...
- [[syntheses/BP-地图建模与决策规范|BP 地图建模与决策规范]]：说明地图实体、赛季索引、版本 meta 和英雄 map-fit 的更新边界。

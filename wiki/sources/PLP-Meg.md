# PLP 来源摘要: Meg

## 来源信息

- 标题：Best build for Meg | Brawl Stars loadout, gears & counters | PL Prodigy
- 来源：[Meg | Power League Prodigy](https://powerleagueprodigy.com/meg)
- 读取日期：2026-06-30
- 上游 raw：[[../../raw/sources/pl-prodigy/brawlers/meg-2026-06-30.md]]
- source_quality：direct_raw_capture
- source_type：third_party_competitive_guide
- payload_source_updated_at：2026-05-25T17:15:02Z

## 可用范围

- usable_for: build_candidates, mode_fit_candidates, matchup_candidates, slot_notes_candidates
- not_usable_for: stable_official_mechanics, final_counter_claim, current_meta_strength_without_overlay, unconditional_bp_recommendation

## 结构化字段摘要

- 推荐 Gadget: `Jolting Volts`
- 推荐 Star Power: `Heavy Metal`
- 推荐 Gears: Shield, Damage
- 推荐 Modes: Gem Grab, Brawl Ball, Hot Zone, Heist
- Notes: none observed
- Avoid: none observed

## Matchup 候选

- countersThese: `Bolt`, `Jae-Yong`, `Nani`, `Piper`, `Glowy`, `Squeak`, `Sprout`, `Byron`
- counteredBy: `Rosa`, `Bibi`, `Sirius`, `Lumi`, `Damian`, `Nita`, `Larry & Lawrie`, `Edgar`

这些 matchup 只作为 `conditional_matchup_seed`，不能写成无条件克制。进入 BP 模型前必须补机制、地图条件、build 条件、失效条件和 BP 用途。

## 与本地 wiki 的意义

- 本页为 [[entities/brawlers/Meg|Meg]] 的 build、mode fit、matchup 候选提供竞技信号。
- 与 Fandom 机制事实发生冲突时，应保留来源差异；PLP 不覆盖稳定机制事实。

## 关联页面

- [[sources/Fandom-Meg|Fandom 来源摘要: Meg]]
- [[entities/brawlers/Meg|Meg]]

# Fandom 来源摘要: Larry & Lawrie

## 来源信息

- 标题：Larry & Lawrie
- 来源：[Larry & Lawrie | Brawl Stars Wiki | Fandom](https://brawlstars.fandom.com/wiki/Larry_%26_Lawrie)
- 读取日期：2026-07-17
- Fandom 页面最后编辑：2026-07-05T02:24:01Z
- 分类：Brawlers / Fandom hero page
- 上游 raw：[[../../raw/sources/fandom/heroes/larry-lawrie.md]]
- source_quality：direct_raw_capture
- source_type：official_or_wiki_mechanics

## 可用范围

- usable_for: stable_mechanics, ability_candidates, build_candidates_from_tips, mode_fit_candidates, map_feature_candidates
- not_usable_for: current_meta_strength_without_overlay, final_counter_claim, unconditional_bp_recommendation

## 页面核心字段

- 稀有度: Epic
- 官方定位: Artillery
- 移动速度: 770 (Fast)<br>924 (with Hypercharge)<br> 3000 (with Order: Fall Back)
- 生命值: 3000
- 攻击距离: 7.33 (Long)
- 装填: 2.2 seconds (Very Slow)
- 普攻数值: 700
- Super 距离: 5
- Super 数值: 3300
- Gadgets: Order: Swap, Order: Fall Back

## BP 建模可抽取信号

- `普攻 / Super / Gadget / Star Power / Hypercharge` 可以拆成稳定机制原子。
- `Tips / Recommended Build` 只能进入候选层；如果涉及模式或地图，后续必须转成 objective contract 或 map feature hook。
- 本页不直接生成 counter 或 pick 顺位结论。

## 抓取覆盖

- Lead excerpt
- Attack: Ticket Dispenser
- Super: Call For Backup
- Gadgets / Order: Swap
- Gadgets / Order: Fall Back
- Star Powers / Protocol: Protect
- Star Powers / Protocol: Assist
- Hypercharge: The Three-Bot Problem
- Tips / Game Modes and Maps
- Tips / Recommended Build
- Tips / Other

## 与本地 wiki 的意义

- `Larry & Lawrie` 的 Fandom 页面已有 direct raw，可作为稳定机制来源。
- 后续升级 [[entities/brawlers/Larry & Lawrie|Larry & Lawrie]] 时，应优先从本 raw 抽取机制事实，再与 [[sources/PLP-Larry-Lawrie|PLP 竞技信号]] 分层合并。

## 关联页面

- [[entities/brawlers/Larry & Lawrie|Larry & Lawrie]]
- [[sources/PLP-Larry-Lawrie|PLP 来源摘要: Larry & Lawrie]]

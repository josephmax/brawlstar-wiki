...
- countersThese: `Mr. P`, `Jessie`, `Gale`, `Sprout`, `Brock`, `Jae-Yong`, `Squeak`, `Nani`
- counteredBy: `Doug`, `Jacky`, `Trunk`, `Frank`, `Finx`, `Bibi`, `Ash`, `Hank`

这些 matchup 只作为 `conditional_matchup_seed`，不能写成无条件克制。进入 BP 模型前必须补机制、地图条件、build 条件、失效条件和 BP 用途。

## 与本地 wiki 的意义

- 本页为 [[entities/brawlers/Shade|Shade]] 的 build、mode fit、matchup 候选提供竞技信号。

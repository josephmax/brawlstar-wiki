# Fandom 来源摘要: Draco

## 来源信息

- 标题：Draco
- 来源：[Draco | Brawl Stars Wiki | Fandom](https://brawlstars.fandom.com/wiki/Draco)
- 读取日期：2026-06-30-v2
- Fandom 页面最后编辑：2026-05-18T17:04:33Z
- 分类：Brawlers / Fandom hero page
- 上游 raw：[[../../raw/sources/fandom/heroes/draco-2026-06-30-v2.md]]
- source_quality：direct_raw_capture
- source_type：official_or_wiki_mechanics

## 可用范围

- usable_for: stable_mechanics, ability_candidates, build_candidates_from_tips, mode_fit_candidates, map_feature_candidates
- not_usable_for: current_meta_strength_without_overlay, final_counter_claim, unconditional_bp_recommendation

## 页面核心字段

- 稀有度: Legendary
- 官方定位: Tank
- 移动速度: 720 (Normal)<br>828 (with Super)<br>864 (with Hypercharge)<br>972 (with Super and Hypercharge)
- 生命值: 5600
- 攻击距离: 4 (Normal)
- 装填: 1 second (Very Fast)
- 普攻数值: 700
- Super 距离: 5.67 (Normal)<br>9 (with Hypercharge)
- Super 数值: 70
- Gadgets: Upper Cut, Last Stand

## BP 建模可抽取信号

- `普攻 / Super / Gadget / Star Power / Hypercharge` 可以拆成稳定机制原子。
- `Tips / Recommended Build` 只能进入候选层；如果涉及模式或地图，后续必须转成 objective contract 或 map feature hook。
- 本页不直接生成 counter 或 pick 顺位结论。

## 抓取覆盖

- Lead excerpt
- Trait
- Attack: Lance Stab
- Super: Dragon Solo
- Gadgets / Upper Cut
- Gadgets / Last Stand
- Star Powers / Expose
- Star Powers / Shredding
- Hypercharge: Fire and Flames
- Tips / Game Modes and Maps
- Tips / Recommended Build
- Tips / Strategies

## 与本地 wiki 的意义

- `Draco` 的 Fandom 页面已有 direct raw，可作为稳定机制来源。
- 后续升级 [[entities/brawlers/Draco|Draco]] 时，应优先从本 raw 抽取机制事实，再与 [[sources/PLP-Draco|PLP 竞技信号]] 分层合并。

## 关联页面

- [[entities/brawlers/Draco|Draco]]
- [[sources/PLP-Draco|PLP 来源摘要: Draco]]

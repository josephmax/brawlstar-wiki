...
bp_brawler_profile:
  ...
  capability_vector:
    ...
    crowd_control: "conditional; Toss Up 满速击飞；Unstoppaball 只在激活 Super 后 1 秒免疫负面效果，不覆盖完整 4 秒 Overdrive"
  ...
  build_switches:
    - build: "Bouncy Ball / Unstoppaball / Health, Damage"
      ...
      changes_capabilities:
        ...
        - "Unstoppaball 只在激活 Super 后提供 1 秒 CC immunity；不提高最高速度，也不保护余下 3 秒 Overdrive"
        - "Health/Damage gears 支持回退再进场和接触伤害转化"
      enables:
        - "Bounty 蓝星/长路线压迫"
        - "越墙收残血或打开关键掩体"
        - "用 Super 开启后的 1 秒窗口穿过第一段控制路线"
  ...
  map_feature_hooks:
    ...
    - id: "hot_zone_overdrive_trail_entry_tax"
      ...
      fails_if: "敌方有远程/投掷从圈外清人，或在 Unstoppaball 的 1 秒免控结束后保留 CC"
      example_maps:
  ...
  failure_modes:
    ...
    - id: "cc_stops_contact_plan"
      ...
      exposed_by: "Fandom 当前机制确认 Unstoppaball 只在激活 Super 后 1 秒免控，余下 Overdrive 仍会被 slow、stun、knockback、pull 或 Cocoon 打断"
      mitigation: "把 Unstoppaball 只预算为穿过第一段控制的 1 秒窗口；后续 CC 必须先骗掉、由队友分担，或将 Bolt 放到最后手"
  ...
  conditional_matchups:
    ...
    - target: ["Gale", "Charlie", "Buzz", "Willow"]
      ...
      fails_when: "控制被 bait，Bolt 在 Unstoppaball 开启后的 1 秒内完成关键接触，或队友从另一路分散控制"
...
```json
{
  "combat_breakpoint_profile": {
    ...
    "damage_packets": [
      {
        "id": "main.max_speed_impact",
        "ability_kind": "main_attack",
        "packet_unit": "impact",
        "delivery_variant": "max_resource",
        "repeat_model": "resource_gated",
        "damage": {"amount": 740, "at_power_level": 1, "scaling": "standard"},
        "active_when": "当前英雄页所示最大移速碰撞；真实伤害随移速连续变化",
        "source_conflict_status": "patch_after_760_conflicts_with_current_raw_740",
        "source_ref": "[[sources/Fandom-Bolt|Fandom-Bolt]]"
      }
    ],
    "defense_modifiers": []

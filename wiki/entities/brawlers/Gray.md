...
Gray 的 BP 价值来自“把地图路线改写成我方可用、敌方不舒服的路径”。他的传送门能让队友绕过正面火力、快速进出目标点或把短手送到敌方后排；`Walking Cane` 能拉关键目标、打断持球或把低血量目标拉出掩体。`Grand Piano` 现在会在 1.5 秒后落下，以与普攻相同的 1280 基础伤害击退并破坏两格半径内障碍，固定墙点的地形转换更可执行，但仍不是无条件开图。错误传送依然可能把队友送进高爆发近战面前。
...
bp_brawler_profile:
  ...
  capability_vector:
    ...
    burst: "中等；Power 11 普攻单发 2560，未带盾/减伤的基础血量 <=5120 为两发线、<=7680 为三发线；Grand Piano 同伤害但有 1.5 秒落点延迟"
    ...
    wall_break: "条件性；Grand Piano 1.5 秒后在 2 格半径破障碍并击退，适合处理固定墙点但仍要服务地形状态计划"
  ...
  build_switches:
    ...
    - build: "Grand Piano / New Perspective"
      ...
      changes_capabilities:
        - "Grand Piano 在瞄准点标记后 1.5 秒落下，造成与普攻相同的 1280 基础伤害、击退和 2 格半径破墙"
  ...
  map_feature_hooks:
    ...
    - id: "selective_wall_transform"
      ...
      route_or_position: "用 1.5 秒落地的 Grand Piano 打开保护投掷/侧墙口袋/球门屏障附近的关键墙"
      objective_conversion: "拆掉敌方口袋或打开进球/长线角度"
      active_when: "目标墙和路线固定，1.5 秒落点不影响地形转换，且开墙后我方远程或传送路线明显受益"
      fails_if: "己方也依赖墙体生存、移动目标能离开落点，或 Pinball Dreams 这类墙本身是己方得分资源"
  ...
  failure_modes:
    ...
    - id: "wall_break_backfires"
      ...
      bp_use: "terrain_state_plan_check"
    - id: "grand_piano_delayed_target_escape"
      active_when: "把 Grand Piano 当即时击退/伤害，用来打可自由横移的敌人而不是固定墙点、窄口或目标位置"
      exposed_by: "[[sources/Fandom-Gray|Fandom-Gray]] Grand Piano 1.5-second landing delay"
...
```json
{"combat_breakpoint_profile":{"schema":"brawler_breakpoint_profile.v1","brawler":"Gray","target_states":[{"id":"body","entity_class":"brawler_body","roster_target":true,"health":{"amount":3400,"at_power_level":1,"scaling":"standard"},"source_ref":"[[sources/Fandom-Gray|Fandom-Gray]]"}],"damage_packets":[{"id":"main.impact","ability_kind":"main_attack","packet_unit":"impact","delivery_variant":"impact","repeat_model":"identical","damage":{"amount":1280,"at_power_level":1,"scaling":"standard"},"active_when":"单发普攻命中；Grand Piano 条件包另行声明","source_conflict_status":"none","source_ref":"[[sources/Fandom-Gray|Fandom-Gray]]"}],"defense_modifiers":[]}}
```

...
bp_brawler_profile:
  ...
  capability_vector:
    ...
    team_support: very_high; Super 提速改变进退/转点，Super Charged Buffie 可给队友护盾，Hyper Buffie 命中可给近旁队友充 Super
    tempo_control: high
    source_trace:
  ...
  build_switches:
    - build: "Sneaky Sneakers / Super Charged / Shield, Damage"
      ...
      changes_capabilities:
        - "Sneaky Sneakers 记录当前位置，3 秒后若 Max 仍存活就回到锚点并恢复当时血量，适合做可回收的短时深压"
        - "Super Charged 让 Max 在自身 Super 生效期间每命中一发延长 0.25 秒加速窗口；移动充 Super 属于独立 Trait"
        - "Shield and Damage stabilize mid-range skirmishes while Max keeps tempo"
      enables:
        - team_speed_engage
        - rotation_and_retreat
        - temporary_deep_pressure_with_return_anchor
      mitigates_failure_modes:
        - need_to_force_short_trade
        - slow_team_rotation
      poor_when:
        - "队伍没有能利用加速进场或追击的队友，Max 会只剩中等输出"
        - "return marker 可被守点，或 Max 会在 3 秒内被直接击杀而无法回退"
    ...
    - build: "Phase Shifter / Run n' Gun mobility variant"
      ...
      changes_capabilities:
        - "Phase Shifter 提供短距离免伤 dash，用来闪避致命弹道或立即重置站位"

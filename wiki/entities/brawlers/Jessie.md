...
Jessie 的 BP 价值来自“站位惩罚 + 炮台据点”。9 格主攻在命中后会向 6.67 格内最近目标弹射，最多打 3 人，能够惩罚抱团、墙后和草内目标；Scrappy 炮台可以越墙部署、挡弹、占线并配合 `Spark Plug` 在 4.33 格半径减速。她的主弹当前弹速为 3050，但满射程仍需预判，弹射后的子弹只有 1522 且距离更短；炮台也仍怕高效清宠/投掷/穿透和强突进。
...
bp_brawler_profile:
  ...
  capability_vector:
    ...
    projectile_reliability: "medium; 主弹弹速 3050，9 格满射程打高机动目标仍需预判；命中后 1522 弹速的弹射可追墙后/草内目标"
  ...
  map_feature_hooks:
    ...
    - id: "bounce_chain_bush_and_group_reveal"
      ...
      fails_if: "敌方严格分散站位，或 Jessie 在 9 格距离的主弹被高机动目标反复躲掉；弹射弹道更慢也不能代替首发命中"
      example_maps:
  ...
  failure_modes:
    ...
    - id: "slow_projectile_into_mobile_targets"
      active_when: "敌方在满射程长距离横移、保持分散，且 Jessie 没有炮台/Spark Plug 先限制路线"
      exposed_by: "Fandom current mechanics: direct projectile speed 3050, bounce speed 1522 with reduced range"
      mitigation: "用炮台/Spark Plug 限走位，选择 chokepoint 地图，并将弹射视为首发命中后的额外收益"

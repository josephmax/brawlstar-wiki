...
bp_brawler_profile:
  ...
  capability_vector:
    ...
    sustained_dps: "medium_chip_high_tempo; 1.6 秒 normal reload 可维持毒压，但单次 raw damage 仍低"
    ...
    survivability: "low_by_health_medium_with_tools; Power 11 本体 5600 HP，满 Shield gear 为 6500；这会把 Bibi 的 Power 11 完整挥棒从两发线恢复到三发线，但前提是接触前护盾未被消耗；Instapoison、Super escape 和 Extra Toxic 另补条件容错"
    ...
    area_control: "medium; 普攻毒伤持续 3 ticks，治疗压制在最后一次命中后维持 4 秒；能延后重置、暴露草中目标，但不是长期封区"
  ...
  map_feature_hooks:
    ...
    - map_feature_type: "anti_heal_slow_zone_entry"
      ...
      active_when: "enemy plan needs healing, grouped entry, or close-range bodies to walk through a predictable lane, and allies can convert within the 3-tick poison / 4-second anti-heal window"
...
```json
{
  "combat_breakpoint_profile": {
    ...
    "target_states": [
      {
        ...
        "roster_target": true,
        "health": {"amount": 2800, "at_power_level": 1, "scaling": "standard"},
        "source_ref": "[[sources/Fandom-Crow|Fandom-Crow]]"
      }
    ],
    "damage_packets": [
      {
        "id": "main.one_dagger_direct",
    ...
    "excluded_temporal_defenses": [
      {
        "id": "instapoison_dynamic_barrier",
        "reason": "护盾取决于当前剩余毒伤并持续衰减，需时间轴与中毒目标状态",
        "source_ref": "[[sources/Fandom-Crow|Fandom-Crow]]"
      }
    ]
  }

...
bp_brawler_profile:
  ...
  capability_vector:
    ...
    crowd_control: "knockback_and_slow_variant; Super 击退，Shell Shock 让命中目标受到 55% slow 持续 2 秒"
    source_trace:
  ...
  build_switches:
    - build: "Fast Forward / Band-Aid / Shield, Damage, Speed, Gadget Cooldown"
      ...
      changes_capabilities:
        - "Fast Forward 朝目标方向 dash 2.67 格并瞬间回满弹药，16 秒 cooldown；不能穿墙/水且会被 stun/pull/knockback 取消"
    ...
    - build: "Clay Pigeons / Shell Shock variant"
      ...
      changes_capabilities:
        ...
        - "Shell Shock 让 Super 命中目标受到 55% slow 持续 2 秒，显著收窄被击退者横移逃离窗口，便于接 Clay Pigeons 或队友追击"
      enables:
        - "中距离收割被击退目标"
        - "对逃跑长手的追击"
        - "Super 后循环再充能"
  ...
  failure_modes:
    ...
    - id: "fast_forward_route_blocked_or_cancelled"
      active_when: "Fast Forward 被 stun/pull/knockback 取消、撞墙/水中断，或 16 秒 cooldown 内被要求再次承担接近/回满弹任务"
      exposed_by: "Fandom Fast Forward mechanics"
      mitigation: "只用在已确认路线和控制已交的时机，不把它当跨地形或连续位移工具；Gadget Cooldown gear 只提高可用频率，不改变路线限制"

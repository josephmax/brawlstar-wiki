...
bp_brawler_profile:
  ...
  build_switches:
    - build: Bottomless Mags / Mission Swimpossible / Shield, Damage
      ...
      changes_capabilities:
        - Bottomless Mags 补一发弹药并创造弹壳，降低空弹期；冷却 22 秒，不能覆盖每次弹壳循环
    ...
    - build: You Only Brawl Twice anti-aggro variant
      ...
      changes_capabilities:
        - 吸收弹壳转护盾并击退附近敌人，适合防一波强突进；冷却 18 秒且会消耗当前全部弹壳
      enables:
        - close_entry_peel
  ...
  map_feature_hooks:
    ...
    - map_feature_type: brawl_ball_last_ammo_slow_or_shell_knockback
      ...
      active_when: Pierce 有弹壳缓存或最后一发资源，You Only Brawl Twice 的 18 秒资源已转好，且队友能接住控制窗口
  ...
  failure_modes:
    - id: shell_resource_tax
      ...
      mitigation: 只在弹壳落点可控、中距离目标路线明确时选；Bottomless Mags 冷却 22 秒，只能救一次关键空弹期

...
- 初始阶段移速慢、射程一般且装填慢，必须先获得 Super 或用 `Power Surge` 临时补阶段。
- `To the Max!` 让攻击撞墙也能分裂，是墙体/走廊图的条件化输出增强。

## 超级技能特征

- Super 让 Surge 短距离跳跃，空中期间免疫大多数直接伤害，落地造成伤害和击退并升级。
- `Serve Ice Cold` 让 Surge 在获得一次 Stage 1 后复活、进球后或 Knockout 回合后保留速度升级。
...
bp_brawler_profile:
  ...
  build_switches:
    - build: Power Surge / Serve Ice Cold / Shield + Damage
      ...
      changes_capabilities:
        - Power Surge 临时给 1 阶段，缓解开局短板
        - Serve Ice Cold 让速度升级成为全局稳定资产，尤其适合 Brawl Ball/Gem Grab 重复回合
        - Shield/Damage 提高低血与爆发对拼容错
      enables:
        - brawl_ball_speed_reset
      ...
      bp_use: 默认竞技 build；需要先判断第一阶段是否能安全取得或由 Gadget 补足
    ...
    - build: To the Max! wall_split_variant
      ...
      mitigates_failure_modes:
        - 目标躲在墙后或窄口不直接露头
      poor_when:
        - 地图开阔或更需要 Serve Ice Cold 保速度
      bp_use: 墙体走廊图的条件 build，不是默认替换 Serve Ice Cold

  map_feature_hooks:
    - map_feature_type: brawl_ball_stage_speed_and_anti_aggro
      uses_feature_by: Serve Ice Cold 保速度，Super 可击退持球人或跳过小墙，近身爆发惩罚坦克推进
      objective_conversion: 抢球、反推进、得分窗口和门前防守
      active_when: 球路需要中距离对拼或反近身，Surge 有 Stage 1/2 或 Power Surge 可启动
    ...
    - map_feature_type: wall_split_choke_pressure
      uses_feature_by: To the Max! 或 Hypercharge 分裂让墙角/窄口附近的敌人难以只靠掩体躲避
  ...
  failure_modes:
    - id: stage0_tempo_tax
      ...
      mitigation: Power Surge 启动、队友保线、选择可充 Super 的中距离地图
    ...
    - id: reset_sensitive_modes
      active_when: Knockout 回合结束、Brawl Ball 进球或死亡导致阶段回退，且未触发 Serve Ice Cold 稳定速度
      exposed_by: Fandom Super/Star Power 说明
      mitigation: 使用 Serve Ice Cold，或把 Surge 作为后手对位而非整局唯一核心

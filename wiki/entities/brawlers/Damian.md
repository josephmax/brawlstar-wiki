...
Damian 是高血、短手、靠受伤充能和进场资源打目标的坦克。他的普攻命中两次后获得强化攻击，强化攻击击退并标记目标，随后爆炸可连锁；Super 让他跳过障碍/水域落地生成 5 秒 `Mosh Pit`，用 speaker 边界把敌人向中心弹回。每个 speaker 从内侧弹飞一名敌人后会立即破碎，所以边界是一次性的分段控制，不是完整 5 秒的持续囚笼。BP 里不能只看“Tank + 跳跃”就认为他能无脑进场：短射程会被开阔长手风筝，强化条会随时间衰减且空拳会清空，敌人被弹起期间也会短暂无敌，需要队友和 Damian 的下一拳衔接。
...
bp_brawler_profile:
  ...
  map_feature_hooks:
    - map_feature_type: gem_super_escape_or_carrier_disruption
      uses_feature_by: "Super 跳跃可越障碍/水域逃生或切入敌方 carrier，Mosh Pit 分段边界限制追击路线；每次内侧弹飞会消耗对应 speaker"
    ...
    - map_feature_type: hot_zone_mosh_pit_body_and_boundary
      uses_feature_by: "Mosh Pit 的一次性 speaker 分段和强化攻击把敌方站区身体向中心拉回，Hypercharge 火区进一步惩罚聚集"
      ...
      fails_if: "关键 speaker 已在第一次弹飞后破碎、边界被远程打掉，或敌方用长手/投掷完全不进圈"
      example_maps:
        - Dueling Beetles
        - Open Business
        - Ring of Fire
  ...
  objective_contracts:
    ...
    - mode: "Hot Zone"
      can_fulfill:
        - "zone_body"
        - "entry_displacement"
        - "single_use_segment_boundary_control"
      cannot_fulfill:
  ...
  failure_modes:
    ...
    - id: mosh_pit_boundary_consumed_or_misconverted
      active_when: "speaker 从内侧弹飞一名敌人后立即破碎、被远程打掉，或敌人弹起期间短暂无敌导致队友伤害空掉"
      exposed_by: "[[sources/Fandom-Damian|Fandom-Damian]] mosh pit speaker and airborne notes"
      mitigation: "把 Mosh Pit 当一次性的分段 route control；优先让关键 speaker 对高价值目标触发，队友等待落点补伤害，不预算同一段重复拦截"

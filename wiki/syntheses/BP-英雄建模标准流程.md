...
6. 跑质量审计：用 `skills/brawl-stars-bp-knowledge-maintenance/scripts/audit_bp_profile_quality.py` 检查明显占位符、缺字段、来源追溯和 slot notes。审计结果默认输出到终端；需要落盘时写入 `outputs/bp-profile-quality-audit.md`，不要写回 `wiki/syntheses/`。

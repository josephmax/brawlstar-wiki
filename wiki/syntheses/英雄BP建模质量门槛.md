...
- `conditional_matchup_seeds` 可以仍是 seed，但每组 seed 至少要写出机制类和激活/失效条件，不得停留在 `mechanism: pending`。
...
- 不含自动草案占位词：`pending`、`unknown_pending`、`needs_review`、`seed_only_not_final`、`PLP mode fit is a seed`、`not_inferred_from_source`。
...
- 版本强度、临时 meta 和赛事复盘应进入版本 / meta overlay，不写成稳定英雄事实。

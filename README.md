...
请从用户给定的地图池中随机选一张地图，并读取对应 `wiki/entities/maps/` 地图实体页来确定 mode。不要要求选手子 agent 读取 `wiki/syntheses/`；syntheses 只用于维护者讨论，不是 skill 运行时输入。
...
- 子 agent 使用 $brawl-stars-bp-slot-decision：先按 `compile` 模式基于地图实体页、英雄实体页和强度输入生成 `runtime_bp_index`，再按 `decide` 模式消费该索引进行 ban / pick 思考。
...
请按 skill 要求先执行 `compile`：读取 `references/compile-knowledge.md`、相关地图实体页、相关英雄实体页和本次 strength_context，生成 `runtime_bp_index`。随后执行 `decide`：读取 `references/runtime-decision-knowledge.md` 和 runtime index，不要读取 `wiki/syntheses/`。输出：
- context_summary
- hard_gate_result

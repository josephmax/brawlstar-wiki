...
执行全量英雄 BP 建模、补抓 Fandom/Power League Prodigy 英雄详情页、扩展到当前 105 位英雄、或批量升级 `wiki/entities/brawlers/` 时，必须先读取 `wiki/syntheses/英雄BP建模升级任务计划.md`。该任务必须先建立 105 行 roster manifest，再分批保留 raw；禁止直接批量生成 BP-ready 字段。

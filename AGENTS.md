...
执行全量英雄 BP 建模、补抓 Fandom/Power League Prodigy 英雄详情页、扩展英雄覆盖、地图 source ingest、或批量升级 `wiki/entities/brawlers/` / `wiki/entities/maps/` 时，必须使用 `skills/brawl-stars-bp-knowledge-maintenance/`。先读该 skill 的 `SKILL.md`，再按任务读取 `references/source-ingest.md`、`brawler-modeling.md`、`map-modeling.md`、`audit-and-validation.md` 或 `runtime-boundary.md`。`wiki/syntheses/BP-英雄建模标准流程.md` 和 `wiki/syntheses/BP-维护归档.md` 只作为维护背景 / 历史归档，不是执行入口。该任务必须先读取 roster manifest，再按当前 BP-active 英雄集合分批保留 raw；已下架或无有效来源覆盖的 roster 行不进入 BP 英雄集合、PLP 缺口追踪、对位边或运行时编译索引。禁止直接批量生成 BP-ready 字段。

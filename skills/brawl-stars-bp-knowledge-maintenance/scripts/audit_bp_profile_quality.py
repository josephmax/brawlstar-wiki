...
def render_markdown(rows: list[dict[str, object]]) -> str:
    ...
    lines = [
        ...
        "",
        "本报告由 `skills/brawl-stars-bp-knowledge-maintenance/scripts/audit_bp_profile_quality.py` 根据 [[syntheses/BP-英雄建模标准流程|BP 英雄建模标准流程]] 生成。它只审计结构和明显占位符，不替代人工机制复核；默认不写入 `wiki/syntheses/`。",
        "",
        "## 汇总",
        "",
        f"- 审计英雄数：{len(rows)}",

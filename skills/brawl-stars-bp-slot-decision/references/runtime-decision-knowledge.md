...
When a call supplies a recall mask, apply [candidate-mask.md](candidate-mask.md) to that call's candidate roots. Include and capability windows cannot expand it. The mask gives only a retrieval boundary; the tools do not interpret its cause. Related facts and global census counts remain intact; `masked_answered_by` is the additional relation projection inside the supplied window.

...
- `--candidate-mask-file`: optional canonical-ID recall mask for this invocation. It restricts candidate roots before detail construction, independently of why the caller chose that window. Empty IDs mean no roots; invalid masks fail. Read [candidate-mask.md](references/candidate-mask.md) for scope, relation preservation and output fields.
